    ...
    foreach (var directory in directories)
    {
      foreach (var assemblyPath in Directory.EnumerateFiles(directory, "*.dll"))
      {
        ...
        try
        {
          ...
          {
            SpeckleLog.Logger.Information("Checking DLL: {path} => References Speckle.Core: {ref}, KitClass: {kitClass}",
                assemblyPath,
                assembly.IsReferencing(SpeckleAssemblyName),
                kitClass?.FullName ?? "null");
