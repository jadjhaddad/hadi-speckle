...
public partial class ConnectorBindingsAutocad : ConnectorBindings
{
  ...
  public override List<MenuItem> GetCustomStreamMenuItems()
  {
    return new List<MenuItem>
    {
        ...
        {
            Header = "Export IFC",
            Icon = "Publish", // Optional: use any Material icon name
            Action = (streamState) =>
            {
                ExportToIFC();
            }
  ...
  public void ExportToIFC()
  {
    ...
    {
      saveDialog.Filter = "IFC files (*.ifc)|*.ifc";
      saveDialog.Title = "Export Civil 3D model to IFC";
      saveDialog.FileName = "model_export.ifc";

      if (saveDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
      {
        string exportPath = saveDialog.FileName;

        // Save script in Documents instead of Temp
        string scriptPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "AutoExportIFC.scr");

        // Script content with simulated input
        ...
        // Run the script
        doc.SendStringToExecute($"_.SCRIPT \"{scriptPath}\"\n", true, false, false);

        SpeckleLog.Logger.Information($"Script created and executed: {scriptPath}");
      }
    }
  }
}
