...
public partial class ConnectorBindingsAutocad : ConnectorBindings
{
      ...
      using (Transaction tr = Doc.Database.TransactionManager.StartTransaction())
      {
        ...
        foreach (var autocadObjectHandle in state.SelectedObjectIds)
        {
          ...
          try
          {
            ...
            converted = ConvertMeshToPolysurface(converted); // 🔁 Post-process mesh to polysurface
            if (converted == null)
            {
  ...
  private Base ConvertMeshToPolysurface(Base original)
  {
    if (original is not Mesh mesh)
      return original;
    ...
    {
      int numVertices = mesh.faces[i++];
      if (numVertices != 3 && numVertices != 4)
        continue;

      var indices = mesh.faces.Skip(i).Take(numVertices).ToArray();
      i += numVertices;

      var flatPts = new List<double>();
      foreach (var idx in indices)
      {
        flatPts.Add(mesh.vertices[3 * idx]);
        flatPts.Add(mesh.vertices[3 * idx + 1]);
        flatPts.Add(mesh.vertices[3 * idx + 2]);
      }

      if (flatPts.Count >= 9) // At least 3 points (x, y, z) each
      {
        var polyline = new Objects.Geometry.Polyline
        {
          value = flatPts,
          closed = true,
          units = mesh.units
        };
        surfaces.Add(polyline);
      }
    }
    ...
    // Create a collection and attach the surfaces
    var polysurface = new Collection("Polysurface", "polysurface");
    polysurface.elements = surfaces;
    polysurface["sourceMesh"] = mesh;
