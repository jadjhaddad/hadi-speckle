...
public class ObjectsKit : ISpeckleKit
{
  ...
  public ISpeckleConverter LoadConverter(string app)
  {
    try
    {
      ...
      SpeckleLog.Logger.Information($"🔍 Trying to load converter for app: {app}");
      SpeckleLog.Logger.Information($"🔧 Available converters: {string.Join(", ", GetAvailableConverters())}");
  ...
  private static ISpeckleConverter LoadConverterFromDisk(string app)
  {
    ...
    {
      SpeckleLog.Logger.Information($"❌ No converter instance matched for {app}. Check GetServicedApplications.");
  ...
  public List<string> GetAvailableConverters()
  {
    ...
    foreach (var converter in allConverters)
    {
      ...
      if (assemblyName.Version.Major == objects.Version.Major && assemblyName.Version.Minor == objects.Version.Minor)
      {
        ...
        SpeckleLog.Logger.Information($"🧩 Found converter DLL: {converter}");
      }
      else
      {
        SpeckleLog.Logger.Warning($"❌ Skipped converter (version mismatch): {converter} (v{assemblyName.Version})");
      }
    }
    ...
    SpeckleLog.Logger.Information("✅ Extracted converter names: {Converters}", string.Join(", ", finalList));

    // 🔧 Manual fallback to include ETABS22 if it's not detected
    if (!finalList.Contains("ETABS22"))
    {
      finalList.Add("ETABS22");
      SpeckleLog.Logger.Warning("🛠️ Manually added 'ETABS22' to converter list as fallback.");
    }
