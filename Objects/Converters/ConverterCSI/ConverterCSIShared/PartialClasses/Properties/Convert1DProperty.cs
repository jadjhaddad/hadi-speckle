...
public partial class ConverterCSI
{
  ...
  .Information($"[Property1DToNative] Converting section '{property1D.name}' with profile type '{property1D.profile?.GetType().Name}'");
    ...
    if (Property1DExists(property1D.name))
    {
      SpeckleLog.Logger
        .ForContext<ConverterCSI>()
        .Information($"[Property1DToNative] Section '{property1D.name}' already exists in ETABS. Skipping creation.");
      return property1D.name;
    }
    ...
    )
    {
      ...
      if (success != 0)
      {
        SpeckleLog.Logger
          .ForContext<ConverterCSI>()
          .Error($"[Property1DToNative] Failed to create circular section '{property1D.name}'");
