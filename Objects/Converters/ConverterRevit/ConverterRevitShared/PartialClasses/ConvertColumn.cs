...
public partial class ConverterRevit
{
  public ApplicationObject ColumnToNative(Column speckleColumn)
  {
    ...
    if (docObj != null)
      //ShowDebugDialog($"🧠 Reusing existing element with ID {docObj.Id} for appId {speckleColumn.applicationId}");
    ...
    var docLevels = new FilteredElementCollector(Doc).OfClass(typeof(DB.Level)).Cast<DB.Level>();

    double baseOffset = 0.0;
    ApplicationObject.State levelState;

    // Base Level: match by name if possible, fallback to geometry
    DB.Level level =
      (speckleColumn.level != null && GetExistingLevelByName(docLevels, speckleColumn.level.name) is DB.Level lvl)
        ? lvl
    ...
    {
      //ShowDebugDialog($"❌ No family symbol found for {speckleRevitColumn.family} : {speckleRevitColumn.type}");
    }
    else
    {
      //ShowDebugDialog("❌ No family symbol found (column was not a RevitColumn)");
    }

    double topOffset = 0.0;

    // Top Level: match by name if available, fallback to endpoint elevation
    DB.Level topLevel =
      (speckleRevitColumn != null && speckleRevitColumn.topLevel != null &&
       GetExistingLevelByName(docLevels, speckleRevitColumn.topLevel.name) is DB.Level topLvl)
        ? topLvl
        : ConvertLevelToRevit(baseLine.GetEndPoint(1), out levelState, out topOffset);
    ...
    if (docObj != null)
    {
      try
      {
        ...
        {
          //ShowDebugDialog($"⚠️ Family mismatch: deleting element {docObj.Id}.\nExisting: {revitType.FamilyName}, Incoming: {familySymbol.FamilyName}");
        ...
        //ShowDebugDialog($"✅ Updated existing column {revitColumn.Id} with new geometry and type.");
      }
      catch (Autodesk.Revit.Exceptions.ApplicationException ex)
      {
        //something went wrong, re-create it
        //ShowDebugDialog($"❌ Failed to update existing element {docObj.Id}: {ex.Message}\nWill attempt to recreate.");
    ...
    if (revitColumn == null && isLineBased)
    {
      ...
      //ShowDebugDialog($"✅ Created new line-based column: {revitColumn.Id}");
    }
    ...
    if (speckleRevitColumn != null && revitColumn == null && !isLineBased)
    {
      ...
      //ShowDebugDialog($"✅ Created new point-based column: {revitColumn?.Id}");
    }
    ...
    if (revitColumn == null)
    {
      //ShowDebugDialog("❌ Native column creation failed. revitColumn is null. Will fallback to DirectShape.");
    ...
    //ShowDebugDialog($"🎉 Final status: {(isUpdate ? "Updated" : "Created")} column {revitColumn.Id}");
