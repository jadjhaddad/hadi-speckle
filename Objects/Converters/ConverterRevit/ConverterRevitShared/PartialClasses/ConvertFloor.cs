...
public partial class ConverterRevit
{
  public ApplicationObject FloorToNative(BuiltElements.Floor speckleFloor)
  {
    ...
    if (docObj != null)
      //ShowDebugDialog($"🧠 Reusing existing element with ID {docObj.Id} for appId {speckleFloor.applicationId}");
    ...
    {
      //ShowDebugDialog("❌ Floor outline is null. Skipping conversion.");
    ...
    if (floorType == null)
    {
      //ShowDebugDialog("❌ No matching FloorType found. Cannot create floor.");
    ...
    if (docObj != null)
    {
      //ShowDebugDialog($"⚠️ Deleting existing floor with ID {docObj.Id} before recreation.");
    ...
    if (revitFloor == null)
    {
      //ShowDebugDialog("❌ Revit floor creation failed. revitFloor is null. Fallback to DirectShape likely.");
    }
    else
    {
      //ShowDebugDialog($"✅ Created native Revit floor. ID: {revitFloor.Id}");
    }
    ...
    {
      //ShowDebugDialog($"⚠️ Could not create floor openings: {ex.Message}");
    ...
    //ShowDebugDialog($"🎉 Final status: Floor created with ID {revitFloor.UniqueId}");
