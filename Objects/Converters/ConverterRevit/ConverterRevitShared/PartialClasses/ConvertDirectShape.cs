...
public partial class ConverterRevit
{
  ...
  {
    //TaskDialog.Show(
    //  $"📦 Entered DirectShapeToNative\nSpeckle Type: {speckleDs.speckle_type}\nName: {speckleDs.name ?? "null"}\nBaseGeometries: {speckleDs.baseGeometries.Count}",
    //  "Debug: DirectShape Entry"
    //);
      ...
      .ForEach(b =>
      {
        switch (b)
        {
          ...
          case Mesh mesh:
            ...
            {
              //TaskDialog.Show(
              //  $"⬛ DirectShape received mesh directly.\nMesh units: {mesh.units}",
              //  "Debug: Mesh fallback"
              //);
          ...
          default:
            TaskDialog.Show(
              $"⚠️ Unsupported geometry in DirectShape:\nType: {b.GetType().Name}",
              "Debug: DirectShape Unsupported Geometry"
            );
    ...
    {
      //TaskDialog.Show(
      //  $"✅ Creating new Revit DirectShape\nCategory: {bic}\nGeometry Count: {converted.Count}",
      //  "Debug: Revit DS Creation"
      //);
      ...
      //TaskDialog.Show("Debug: Post SetShape", $"🧱 SetShape call completed.\nConverted geometry count: {converted.Count}\nRevit DS Id: {revitDs.Id}");
      ...
      var addedElement = Doc.GetElement(revitDs.Id);
      if (addedElement == null)
      {
        //TaskDialog.Show("❌ Debug: Element Missing", "The DirectShape was not added to the document.");
      }
      else
      {
        //TaskDialog.Show("✅ Debug: Element Added", $"Element Id: {addedElement.Id} | Category: {addedElement.Category?.Name}");
      }
