...
public partial class ConverterRevit
{
  ...
  {
    SpeckleLog.Logger.Information($"📥 Incoming Speckle Level, Name: {speckleLevel.name}\nElevation: {speckleLevel.elevation}\nUnits: {speckleLevel.units}\nAppId: {speckleLevel.applicationId}");
    ...
    if (GetExistingElementByApplicationId(speckleLevel.applicationId) is DB.Level existingLevelAlreadyReceived)
    {
      ...
      if (revitLevel.Name != speckleLevel.name)
      {
        // check for name conflict before assigning
        var existingNames = new HashSet<string>(docLevels.Select(lvl => lvl.Name));
        if (!existingNames.Contains(speckleLevel.name))
        {
          revitLevel.Name = speckleLevel.name;
        }
        else
        {
          SpeckleLog.Logger.Warning($"⚠️ Could not assign level name '{speckleLevel.name}' — already exists. Keeping '{revitLevel.Name}'.");
        }
      }
      ...
      SpeckleLog.Logger.Information($"Level Match, Matched by Application ID:\nName: {existingLevelAlreadyReceived.Name}\nElevation: {existingLevelAlreadyReceived.Elevation}");
    }
    //match by elevation
    else if (
      !exactElevation
    ...
    )
    {
      ...
      SpeckleLog.Logger.Information($"Level Match, Matched by Closest Elevation:\nOffset: {elevationOffset}");
    }
    //match by elevation
    else if (
      elevationMatch
    ...
    )
    {
      ...
      state = ApplicationObject.State.Updated;
      SpeckleLog.Logger.Information($"Level Match, Matched by Exact Elevation:\nName: {existingLevelWithSameElevation.Name}\nElevation: {existingLevelWithSameElevation.Elevation}");
    }
    else
    {
      ...
      var existingNames = new HashSet<string>(docLevels.Select(lvl => lvl.Name));
      string baseName = speckleLevel.name;
      ...
      revitLevel.Name = newName;

      SpeckleLog.Logger.Information($"Level Created: '{newName}' at elevation {speckleLevelElevation} (original name: '{baseName}')");

      var rl = speckleLevel as RevitLevel;
      if (rl != null && rl.createView)
      {
        CreateViewPlan(newName, revitLevel.Id);
      }
