...
public partial class ConverterRevit : ISpeckleConverter
{
  ...
  private Base SwapGeometrySchemaObject(Base @object)
  {
    ...
    if (speckleSchema is BER.DirectShape ds)
    {
      ...
      var wrappedType = @object.GetType().Name;
      var wrappedId = @object.id;
      var wrappedSchemaId = speckleSchema.id;
      var logMessage = $"⚠️ [DirectShape Fallback]\n" +
                       $"- Wrapped type: {wrappedType}\n" +
                       $"- Wrapped ID: {wrappedId}\n" +
                       $"- DirectShape ID: {wrappedSchemaId}\n" +
                       $"- BaseGeometries count (before): {ds.baseGeometries?.Count ?? 0}";

      //TaskDialog.Show(logMessage, "Speckle Debug");
  ...
  {
    //ShowDebugDialog($"🔍 Attempting to convert {base.GetType()} with id: {base.id}");
  ...
  {
    //ShowDebugDialog($"🔁 ConvertToNativeObject: {@base.speckle_type} | id: {@base.id}");
    ...
    switch (@object)
    {
      ...
      case BER.DirectShape o:
        TaskDialog.Show("⚠️ Routed to DirectShape",
          $"Speckle Type: {o.speckle_type}\n" +
          $"Name: {o.name}\n" +
          $"AppId: {o.applicationId ?? "null"}\n" +
          $"Base geometry count: {o.baseGeometries?.Count ?? 0}");
      ...
      case BE.Pipe o:
        // ✅ Read checkbox setting (stored as string, not bool)
        bool civilPipeAsDs = false;
        if (Settings.TryGetValue("civil-pipes-as-directshape", out var settingStr) &&
            bool.TryParse(settingStr, out var parsedFlag))
        {
          civilPipeAsDs = parsedFlag;
        }

        // 🧠 Logic for Civil3D pipes (identify by metadata or speckle_type)
        var speckleType = o["speckle_type"]?.ToString()?.ToLower();
        var host = o["host"]?.ToString()?.ToLower();
        var metadata = o["__closure"]?.ToString()?.ToLower();

        if (civilPipeAsDs && (
              (speckleType != null && speckleType.Contains("pipe")) ||
              (host != null && host.Contains("civil")) ||
              (metadata != null && metadata.Contains("civil"))
           ))
        {
          if (o["@displayValue"] is List<Objects.Geometry.Mesh> displayMeshes && displayMeshes.Any())
          {
            var ds = new Objects.BuiltElements.Revit.DirectShape
            {
              id = o.id,
              applicationId = o.applicationId,
              name = "Civil3D Pipe",
              baseGeometries = displayMeshes.Select(m => (Base)m).ToList(),
          ...
          var failedAppObj = new ApplicationObject(o.id, o.speckle_type)
          {
            applicationId = o.applicationId
          };
          failedAppObj.Update(
            status: ApplicationObject.State.Failed,
            logItem: "Civil 3D pipe had no @displayValue meshes for DirectShape fallback."
          );
          return failedAppObj;
        }
  ...
  public object ConvertToNativeDisplayable(Base @base)
  {
    ...
    // ✅ Intercept known types before fallback
    if (@base is PolygonElement polygonElement)
    {
      TaskDialog.Show("Debug", "🔁 Routed PolygonElement through PolygonElementToNative.");
      SpeckleLog.Logger.Information("🔁 Routed PolygonElement through PolygonElementToNative.");
      return PolygonElementToNative(polygonElement);
    }

    if (@base is GisFeature gisFeature)
    {
      TaskDialog.Show("Debug", "🔁 Routed GisFeature through GisFeatureToNative.");
      SpeckleLog.Logger.Information("🔁 Routed GisFeature through GisFeatureToNative.");
      return GisFeatureToNative(gisFeature);
    }

    // 🧱 Fallback path (DirectShape-based)
    var nativeObject = DisplayableObjectToNative(@base);
  ...
  public static class ReceiveModeSettings
  {
    public static bool CivilPipesAsDirectShape = true; // Or false by default
  }
