...
public partial class ConverterAutocadCivil : ISpeckleConverter
{
        ...
        case DBObject obj:
          ...
          switch (obj)
          {
            ...
            case AcadDB.Surface o:
              ...
              break;
            case PolyFaceMesh o:
              var rawMesh1 = MeshToSpeckle(o);
              @base = ConvertMeshToPolysurface(rawMesh1);
              break;
            case ProxyEntity o:
              @base = ProxyEntityToSpeckle(o);
              break;
            case SubDMesh o:
              var rawMesh2 = MeshToSpeckle(o);
              @base = ConvertMeshToPolysurface(rawMesh2);
              break;
            case Solid3d o:
              if (o.IsNull)
              {
              ...
              {
                var rawMesh3 = SolidToSpeckle(o, out notes);
                @base = ConvertMeshToPolysurface(rawMesh3);
              }
              break;
  ...
  private Base ConvertMeshToPolysurface(Objects.Geometry.Mesh mesh)
  {
    if (mesh == null || mesh.faces == null || mesh.vertices == null)
      return mesh;

    var surfaces = new List<Base>();
    int i = 0;

    while (i < mesh.faces.Count)
    {
      int faceType = mesh.faces[i++];
      var faceIndices = mesh.faces.Skip(i).Take(faceType).ToList();
      i += faceType;
      ...
      {
        int vi = index * 3;
        if (vi + 2 >= mesh.vertices.Count) continue;
        coords.Add(mesh.vertices[vi]);
        coords.Add(mesh.vertices[vi + 1]);
        coords.Add(mesh.vertices[vi + 2]);
      }

      if (coords.Count >= 9) // at least 3 points
      {
    ...
    var polysurface = new Base();
    polysurface["elements"] = surfaces;      // List<Base> of Polyline faces
    polysurface["sourceMesh"] = mesh;        // Optional: preserve the original mesh
    polysurface["units"] = mesh.units;
    polysurface["name"] = "Polysurface";
